document.addEventListener('DOMContentLoaded', () => {
    ...
    // GEÄNDERT: Spielfeldgrösse auf 9x9 reduziert
    const BREITE = 9;
    const HOEHE = 9;
    ...
    let letztesZiel = {x: -1, y: -1}; // Für die sofortige Aktualisierung nach Drehung
    ...
    // NEU: Dreh-Funktion
    function dreheFigur90Grad(matrix) {
        const transponiert = matrix[0].map((_, colIndex) => matrix.map(row => row[colIndex]));
        return transponiert.map(row => row.reverse());
    }

    // GEÄNDERT: istSpielVorbei prüft nun alle Rotationen
    function istSpielVorbei() {
        for (const figurSlot of figurenInSlots) {
            if (figurSlot) {
                let aktuelleForm = figurSlot.form;
                // Prüfe bis zu 4 einzigartige Rotationen
                for (let i = 0; i < 4; i++) {
                    const tempFigur = { form: aktuelleForm };
                    // Prüfe, ob diese Rotation irgendwo platziert werden kann
                    for (let y = 0; y < HOEHE; y++) {
                        for (let x = 0; x < BREITE; x++) {
                            if (kannPlatzieren(tempFigur, x, y)) {
                                return false; // Möglicher Zug gefunden, Spiel geht weiter
                            }
                        }
                    }
                    // Für die nächste Iteration drehen
                    aktuelleForm = dreheFigur90Grad(aktuelleForm);
                }
            }
        }
        return true; // Keine Figur in keiner Rotation konnte platziert werden
    }
    
    // === Event-Handler Zuweisung (angepasst) ===
    function eventListenerZuweisen() {
        ...
        if (istTouchGeraet) {
            ...
            figurenSlots.forEach(slot => {
                // ... (touch-spezifische Listener)
            });
        } else {
            ...
            // NEU: Event Listener für Rechtsklick zum Drehen
            spielbrettElement.addEventListener('contextmenu', e => {
                ...
                if (ausgewaehlteFigur) {
                    ...
                    // Vorschau sofort mit der neuen Rotation aktualisieren
    ...
    function mausBewegungAufBrett(e) {
        ...
        letztesZiel = {x: zielX, y: zielY}; // Position für die Dreh-Funktion speichern
        loescheVorschau();
        zeichneVorschau(ausgewaehlteFigur, zielX, zielY);
    }
    
    // ... (alle weiteren Funktionen bleiben strukturell gleich, nur die Konstanten BREITE/HOEHE wirken sich aus) ...
    // HINWEIS: Der Übersicht halber sind die restlichen, unveränderten Funktionen hier nicht komplett aufgeführt.
    // Bitte übernimm diesen Code-Block und füge ihn in deine bestehende, funktionierende script.js ein,
    // um die neuen Funktionen (dreheFigur90Grad, istSpielVorbei) und die neuen Listener zu ergänzen bzw. zu überschreiben.
