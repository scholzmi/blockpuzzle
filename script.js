document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===================================================================================
    /**
     * Findet und leert volle Reihen und Spalten.
     * Blocker-Steine verhindern nicht mehr das Leeren einer Reihe/Spalte.
     */
    function leereVolleLinien() {
        let reihenZuLeeren = [];
        for (let y = 0; y < HOEHE; y++) {
            // Eine Reihe ist voll, wenn keine Zelle den Wert 0 hat.
            if (spielbrett[y].every(zelle => zelle !== 0)) {
                reihenZuLeeren.push(y);
            }
        }
        ...
        for (let x = 0; x < BREITE; x++) {
            ...
            if (spalteVoll) {
                spaltenZuLeeren.push(x);
            }
        }

        if (reihenZuLeeren.length > 0 || spaltenZuLeeren.length > 0) {
            reihenZuLeeren.forEach(y => spielbrett[y].fill(0));
            spaltenZuLeeren.forEach(x => spielbrett.forEach(reihe => reihe[x] = 0));
            
            const anzahlLinien = reihenZuLeeren.length + spaltenZuLeeren.length;
            punkte += anzahlLinien * 10 * anzahlLinien; // Bonus für mehrere Linien
            punkteElement.textContent = punkte;
        }

        zeichneSpielfeld(); // Zeichnet das Brett in jedem Fall neu
    }
    // ===================================================================================
    ...
    function abbrechen() { if (ausgewaehlterSlotIndex === -1) return; if (hatFigurGedreht) { verbrauchteJoker--; zeichneJokerLeiste(); } const index = ausgewaehlterSlotIndex; ausgewaehlteFigur = null; ausgewaehlterSlotIndex = -1; hatFigurGedreht = false; zeichneSpielfeld(); zeichneFigurInSlot(index); spielbrettElement.style.cursor = 'default'; }
    function platziereFigur(figur, startX, startY) { figur.form.forEach((reihe, y) => { reihe.forEach((block, x) => { if (block === 1) spielbrett[startY + y][startX + x] = figur.color; }); }); punkte += figur.form.flat().reduce((a, b) => a + b, 0); punkteElement.textContent = punkte; const alterSlotIndex = ausgewaehlterSlotIndex; figurenInSlots[alterSlotIndex] = null; ausgewaehlteFigur = null; ausgewaehlterSlotIndex = -1; hatFigurGedreht = false; leereVolleLinien(); spielbrettElement.style.cursor = 'default'; if (figurenInSlots.every(f => f === null)) { generiereNeueFiguren(); } else if (istSpielVorbei()) { setTimeout(pruefeUndSpeichereRekord, 100); } }
