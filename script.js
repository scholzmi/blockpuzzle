document.addEventListener('DOMContentLoaded', () => {
    ...
    // === Variablen für beide Steuerungsarten ===
    let ausgewaehlteFigur = null;
    let ausgewaehlterSlotIndex = -1;
    let gezogenesElement = null; // Für Drag & Drop
    ...
    // --- Klick-Steuerung Funktionen (Desktop) ---
    ...
    // --- Drag & Drop-Steuerung Funktionen (Touch) ---
    function dragStartTouch(e) {
        e.preventDefault();
        const slot = e.currentTarget;
        ausgewaehlterSlotIndex = parseInt(slot.dataset.slotId, 10);
        if (figurenInSlots[ausgewaehlterSlotIndex]) {
            ausgewaehlteFigur = figurenInSlots[ausgewaehlterSlotIndex];
    ...
    function dropTouch(e) {
        ...
        if (zelle) {
            ...
            if (kannPlatzieren(ausgewaehlteFigur, zielX, zielY)) {
                const slot = e.currentTarget;
                slot.innerHTML = ''; // Figur aus Slot entfernen
