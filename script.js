document.addEventListener('DOMContentLoaded', () => {
    ...
    function generiereNeueFiguren() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (zufallsFigur) {
                // NEU: Figur wird zufällig gedreht, bevor sie im Slot platziert wird.
                let form = zufallsFigur.form;
                const anzahlRotationen = Math.floor(Math.random() * 4);
                for (let r = 0; r < anzahlRotationen; r++) {
                    form = dreheFigur90Grad(form);
                }
                
                figurenInSlots[i] = { form: form, color: zufallsFigur.color, id: i };
    ...
    async function spielStart() { const configGeladen = await ladeKonfiguration(); if (!configGeladen) { spielbrettElement.innerHTML = '<p style="color: red; text-align: center; padding: 20px;">Fehler beim Laden der config.json!</p>'; return; } if (document.body.classList.contains('boss-key-aktiv')) { toggleBossKey(); } const gespeicherterRekord = getCookie("rekord"); rekord = gespeicherterRekord ? parseInt(gespeicherterRekord, 10) || 0 : 0; rekordElement.textContent = rekord; punkte = 0; punkteElement.textContent = punkte; rundenZaehler = 0; verbrauchteJoker = 0; zeichneJokerLeiste(); erstelleSpielfeld(); zeichneSpielfeld(); generiereNeueFiguren(); }
    ...
    function platziereFigur(figur, startX, startY) { let blockAnzahl = 0; figur.form.forEach((reihe, y) => { reihe.forEach((block, x) => { if (block === 1) { spielbrett[startY + y][startX + x] = figur.color; blockAnzahl++; } }); }); punkte += blockAnzahl; punkteElement.textContent = punkte; const alterSlotIndex = ausgewaehlterSlotIndex; ausgewaehlteFigur = null; ausgewaehlterSlotIndex = -1; figurenInSlots[alterSlotIndex] = null; leereVolleLinien(); spielbrettElement.style.cursor = 'default'; if (figurenInSlots.every(f => f === null)) { generiereNeueFiguren(); } else if (istSpielVorbei()) { setTimeout(pruefeUndSpeichereRekord, 100); } }
