document.addEventListener('DOMContentLoaded', () => {
    // ... (DOM-Elemente, Konstanten und Variablen bleiben unverändert) ...

    // === Logik (angepasst) ===

    /**
     * Lädt und verarbeitet die config.json. Liest jetzt auch die "symmetrisch"-Eigenschaft.
     */
    async function ladeKonfiguration() {
        try {
            ...
            const erstelleFigurenPool = (pool) => 
                Array.isArray(pool) ? pool.map(f => ({ 
                    form: parseShape(f.shape), 
                    color: f.color || 'default',
                    symmetrisch: f.symmetrisch || false // NEU: Liest die Symmetrie-Eigenschaft
                })) : [];
    ...
    function eventListenerZuweisen() {
        // ... (andere Listener bleiben unverändert) ...

        spielbrettElement.addEventListener('contextmenu', e => {
            e.preventDefault();
            if (ausgewaehlteFigur) {

                // NEU: Logik zur Joker-Verwendung
                // Ein Joker wird nur verbraucht, wenn die Figur NICHT symmetrisch ist
                // UND sie in dieser Runde noch nicht gedreht wurde.
                if (!ausgewaehlteFigur.symmetrisch && !hatFigurGedreht) {
                    if (verbrauchteJoker >= ANZAHL_JOKER) return; // Keine Joker mehr, Drehen nicht möglich
                ...
                // Die visuelle Drehung findet immer statt (auch bei symmetrischen Figuren).
        ...
        // ... (Rest der Listener-Zuweisung bleibt unverändert) ...
    }
