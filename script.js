document.addEventListener('DOMContentLoaded', () => {
    // === DOM-Elemente ===
    const aenderungsElement = document.getElementById('letzte-aenderung'); // NEU
    ...
    async function ladeKonfiguration() {
        try {
            ...
            // NEU: Setzt den Text für die letzte Änderung
    ...
    (function eventListenerZuweisen() {
        ...
        spielbrettElement.addEventListener('contextmenu', e => { e.preventDefault(); if (ausgewaehlteFigur) { if (!ausgewaehlteFigur.symmetrisch && !hatFigurGedreht) { if (verbrauchteJoker >= ANZAHL_JOKER) return; verbrauchteJoker++; hatFigurGedreht = true; zeichneJokerLeiste(); if (verbrauchteJoker >= ANZAHL_JOKER) { penaltyAktiviert = true; } } ausgewaehlteFigur.form = dreheFigur90Grad(ausgewaehlteFigur.form); zeichneSpielfeld(); zeichneVorschau(ausgewaehlteFigur, letztesZiel.x, letztesZiel.y); } });
        if(anleitungToggleIcon) { anleitungToggleIcon.addEventListener('click', () => { if(anleitungContainer) anleitungContainer.classList.toggle('versteckt'); }); }
        if(infoToggleIcon) { infoToggleIcon.addEventListener('click', () => { if(infoContainer) infoContainer.classList.toggle('versteckt'); }); }
    })();
