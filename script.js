document.addEventListener('DOMContentLoaded', () => {
    // ... (alle Deklarationen am Anfang bleiben unverändert) ...
    
    // ===================================================================================
    // KERNÄNDERUNG: Die neue Vorschau-Logik
    // ===================================================================================
    /**
     * Zeichnet die Vorschau. Erzeugt jetzt den "Chamäleon"-Effekt.
     */
        ...
        figur.form.forEach((reihe, y) => {
            reihe.forEach((block, x) => {
                if (block === 1) {
                    ...
                    // Nur innerhalb des Spielfelds zeichnen
                    if (brettY < HOEHE && brettX < BREITE && brettY >= 0 && brettX >= 0) {
                        ...
                        if (zustandDarunter === 0) {
                            // Feld darunter ist leer: Nimm die Vorschau-Farbe der gehaltenen Figur
                            ...
                            // Feld darunter ist belegt: Nimm die Vorschau-Farbe der belegten Figur
    ...
    // ... (der Rest des Skripts bleibt unverändert, hier zur Vollständigkeit der ganze Code) ...
    ...
    function loescheVorschau() { document.querySelectorAll('.vorschau').forEach(zelle => { zelle.classList.remove('vorschau'); if (!zelle.classList.contains('belegt')) { zelle.style.backgroundColor = ''; } }); }
    function eventListenerZuweisen() { const istTouchGeraet = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0); window.addEventListener('keydown', (e) => { if (e.key === 'Escape') { abbrechen(); } else if (e.key.toLowerCase() === 'b') { toggleBossKey(); } }); if (istTouchGeraet) { figurenSlots.forEach((slot, index) => { slot.dataset.slotId = index; slot.addEventListener('touchstart', dragStartTouch, { passive: false }); }); window.addEventListener('touchmove', dragMoveTouch, { passive: false }); window.addEventListener('touchend', dropTouch); } else { figurenSlots.forEach((slot, index) => { slot.addEventListener('click', () => figurSlotKlick(index)); }); spielbrettElement.addEventListener('mousemove', mausBewegungAufBrett); spielbrettElement.addEventListener('mouseleave', loescheVorschau); spielbrettElement.addEventListener('click', klickAufBrett); spielbrettElement.addEventListener('contextmenu', e => { e.preventDefault(); if (ausgewaehlteFigur) { ausgewaehlteFigur.form = dreheFigur90Grad(ausgewaehlteFigur.form); zeichneVorschau(ausgewaehlteFigur, letztesZiel.x, letztesZiel.y); } }); } }
    function dragStartTouch(e) { e.preventDefault(); const slot = e.currentTarget; const index = parseInt(slot.dataset.slotId, 10); if (figurenInSlots[index]) { ausgewaehlteFigur = figurenInSlots[index]; ausgewaehlterSlotIndex = index; gezogenesElement = slot.querySelector('.figur-container'); if (gezogenesElement) { gezogenesElement.style.position = 'absolute'; gezogenesElement.style.zIndex = '1000'; slot.innerHTML = ''; } } }
