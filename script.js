document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===================================================================================
    // STEUERUNGSLOGIK (mit der wichtigen Änderung in `abbrechen`)
    // ===================================================================================
     ...
     * Gibt bei Bedarf einen verbrauchten Dreh-Joker zurück.
     */
    function abbrechen() {
        if (ausgewaehlterSlotIndex === -1) return;

        // NEU: Joker zurückgeben, falls für diese Figur einer verbraucht wurde
        if (hatFigurGedreht) {
            verbrauchteJoker--;
            zeichneJokerLeiste();
        }
    ...
    // ... (Restlicher Code bleibt unverändert, hier zur Vollständigkeit) ...
    ...
    function leereVolleLinien() { let vR = [], vS = []; for(let y=0; y<HOEHE; y++) { if(spielbrett[y].every(zelle => zelle !== 0 && zelle !== 'blocker')) vR.push(y); } for(let x=0; x<BREITE; x++) { let spalteVoll = true; for(let y=0; y<HOEHE; y++) { if(spielbrett[y][x] === 0 || spielbrett[y][x] === 'blocker') { spalteVoll = false; break; } } if(spalteVoll) vS.push(x); } if (vR.length > 0 || vS.length > 0) { vR.forEach(y => spielbrett[y].fill(0)); vS.forEach(x => spielbrett.forEach(r => r[x] = 0)); const linien = vR.length + vS.length; punkte += linien * 10 * linien; punkteElement.textContent = punkte; } zeichneSpielfeld(); }
